...
- 관련 문서: `MONSTER_CATALOG.md`, `STAT_TREE_CATALOG.md`, `STAGE_CATALOG.md`, `TOWER_BALANCE_BENCHMARK.md`
- 마지막 정리: 2026-08-02
...
아래 여섯 타워는 역할별 희귀도 사다리의 시작점입니다. 이름과 역할 정체성은 확정했고, 세부 수치는 1차 결정론적 벤치마크를 통과한 임시 밸런스 값입니다.
...
세부 가정과 결과는 `TOWER_BALANCE_BENCHMARK.md`에서 관리하고 `tools/balance/tower_baseline.py`로 재현합니다.
...
역할별 최저급 기준 타워 6종을 등록하고 1차 결정론적 벤치마크를 완료했습니다.
...
다음 검증 단계는 스테이지 1 표준 몬스터 3종과 실제 10초 웨이브 생성 간격을 작성한 뒤, 4슬롯 혼합 편성에서 평균 군집도·마무리 발동률·Engage 가동률을 실측하는 것입니다.