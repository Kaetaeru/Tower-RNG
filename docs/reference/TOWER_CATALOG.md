...
- 관련 문서: `MONSTER_CATALOG.md`, `STAT_TREE_CATALOG.md`, `STAGE_CATALOG.md`, `TOWER_BALANCE_BENCHMARK.md`, `STAGE1_WAVE_BENCHMARK.md`
- 마지막 정리: 2026-08-02
...
아래 여섯 타워는 역할별 희귀도 사다리의 시작점입니다. 이름과 역할 정체성은 확정했고, 세부 수치는 1차 솔로·역할 기여도와 2차 혼합 편성 벤치마크를 통과한 임시 밸런스 값입니다.
...
- 역할별 `EquivalentContribution`은 0.970~1.030 범위
- 서로 다른 네 역할의 15개 편성이 스테이지 1 일반 웨이브에서 누수 0
...
세부 가정과 결과는 `TOWER_BALANCE_BENCHMARK.md`, `STAGE1_WAVE_BENCHMARK.md`에서 관리하고 `tools/balance/tower_baseline.py`, `tools/balance/stage1_wave_sim.py`로 재현합니다.
...
- 행동 주기: 1.00초
- 대상 수: 최대 3
- 역할별 기여 계산: 평균 1.67대상 × 0.60 = 유효 DPS 약 1.002
- 솔로 기준 처치시간: 8.25초
- 성능 강화 적용값: 충돌·범위 피해
- 치명타 적용 여부: 예, 대상별 판정
- 이동속도·교전거리: 편대 유지
...
- 역할별 기여 계산: 마무리 공격 비중 40%, Engage 유효 가동률 90% 기준 1.008
- 솔로 기준 처치시간: 5.75초
- 성능 강화 적용값: 기본·마무리 피해
- 치명타 적용 여부: 예
...
- EquivalentContribution 기준값: 1.000
- 1차 벤치마크 결과: 0.995, 통과
- 기준 전투 시나리오: 일반 웨이브 40초와 보스 웨이브 17.5초의 시간 가중 평균
...
- 행동 주기: 2.00초
- 대상 수: 1
- 대형 사냥 효과: 정예·보스 대상 피해 +80%, 최종 2.88
- 역할별 기여 계산: 일반 DPS 0.80을 40초, 보스 DPS 1.44를 17.5초로 시간 가중해 0.995
- 솔로 기준 처치시간: 6.50초
...
- 제작 메모: 기존 +125%는 시간 가중 기여도 1.104로 과도해 1차 벤치마크에서 +80%로 조정

---

## 1차 벤치마크 요약

| 타워 | 솔로 처치시간 | EquivalentContribution | 편차 | 결과 |
|---|---:|---:|---:|---|
| 견습 궁수 | 4.25초 | 1.000 | 0.0% | 통과 |
| 돌팔매꾼 | 8.25초 | 1.002 | +0.2% | 통과 |
| 서리 견습생 | 9.25초 | 1.000 | 0.0% | 통과 |
| 골목 도적 | 5.75초 | 1.008 | +0.8% | 통과 |
| 신참 북잡이 | 9.25초 | 1.000 | 0.0% | 통과 |
| 멧돼지 사냥꾼 | 6.50초 | 0.995 | -0.5% | 통과 |

현재 여섯 타워는 솔로 안전성과 `EquivalentContribution` 허용 범위를 모두 통과합니다.

---

## 2차 스테이지 1 일반 웨이브 요약

세 가지 기준 몬스터와 웨이브 1~4를 사용해 서로 다른 네 역할의 15개 편성을 검증했습니다.
...
- 돌팔매꾼을 포함한 편성은 들쥐 군집 웨이브에서 빠름
- 멧돼지 사냥꾼은 보스가 없는 일반 웨이브에서 특화 피해를 사용하지 못함
- 제어·지원 비중이 높은 편성은 직접 처치 속도가 느리지만 안전 범위 유지

대형 사냥 역할의 최종 기여도는 스테이지 1 보스 작성 후 5웨이브 전체 주기로 다시 검증합니다.
...
역할별 최저급 기준 타워 6종은 다음 두 검증을 통과했습니다.

```text
1차: 솔로 안전성과 EquivalentContribution
2차: 스테이지 1 일반 웨이브 1~4의 15개 혼합 편성
```

다음 검증은 스테이지 1 보스와 웨이브 5를 작성한 뒤 5웨이브 전체 주기에서 대형 사냥 역할과 보상 곡선을 확인하는 것입니다.
