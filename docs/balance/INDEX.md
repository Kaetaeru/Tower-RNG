...
계산 문서는 값을 추천하고, 실제 게임에 들어가는 값은 `docs/catalog`가 별도로 채택합니다.

---

# 계산 문서

기존 계산 문서는 링크 호환을 위해 잠시 `docs/reference`에 남아 있습니다. 새 계산 문서는 `docs/balance`에만 작성합니다.
...
| `../reference/V1_COIN_COMBAT_BENCHMARK.md` | 독립 코인 전투 성장 구성요소 |
| `V1_GATE_ECONOMY_BENCHMARK.md` | 문·슬롯·전투·유연 예산 통합 경제 |

현재 문 가격과 코인 전투 XI의 계산 권고안은 `V1_GATE_ECONOMY_BENCHMARK.md`가 우선합니다. 카탈로그 채택 전까지 구현 확정값으로 간주하지 않습니다.
...
| `../../tools/balance/stage9_wave_sim.py` | 스테이지 9 정글 최종전 |
